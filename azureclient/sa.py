...
class AZStorageAccount:
    def __init__(self, sa_name: str = AZDataPipelineConfig.STORAGE_ACCOUNT_NAME,
                 rg: AZResourceGroup = AZResourceGroup()):

        if not rg.exist:
            raise ResourceNotFoundError(message=f"The resource group ({rg.name}) does not exist")
        self._rg = rg
        self.storage_client = StorageManagementClient(credential=self._rg.credential,
                                                      subscription_id=self._rg.subscription_id)
    ...
    def create(self, location: str = AZDataPipelineConfig.RG_LOCATION,
               kind: str = "StorageV2", sku="Standard_LRS", access_tier: str = "Hot",
               allow_blob_public_access: bool = False,
               minimum_tls_version="TLS1_2", is_hns_enabled: bool = True, large_file_share_enabled: bool = False,
               enable_https_traffic_only=True, tags: dict = AZDataPipelineConfig.TAGS):
    ...
    def update(self, kind: str = "StorageV2", sku="Standard_LRS", access_tier: str = "Hot",
               allow_blob_public_access: bool = False,
               minimum_tls_version="TLS1_2", is_hns_enabled: bool = True, large_file_share_enabled: bool = False,
               enable_https_traffic_only=True, tags: dict = AZDataPipelineConfig.TAGS):
    ...
    def conn_string(self):
        if self.exist:
            ...
            return f"DefaultEndpointsProtocol=https;EndpointSuffix=core.windows.net;" \
